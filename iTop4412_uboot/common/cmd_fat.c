...
int do_fat_fsload (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	sprintf(buf, "%lX", size);
	setenv("filesize", buf);
#if (defined(CONFIG_TA4)) && ( defined(CONFIG_SD_UPDATE)||defined(CONFIG_SD_AUTOUPDATE) )
		{
		char temp[100];
	
		if(size!=-1) {	//file found
	
			if(!strcmp(argv[4], "u-boot.bin") ) {
			
					sprintf(temp, "emmc open 0");
					run_command(temp, 0);
					sprintf(temp, "mmc write 0 40000000 0 400");
					run_command(temp, 0);
					sprintf(temp, "emmc close 0");
					run_command(temp, 0);
					printf("Finished updating %s...\n", argv[4]);
			}
	
			else if(!strcmp(argv[4], "zImage") || !strcmp(argv[4], "zimage")) {
				sprintf(temp, "movi write kernel 40008000");
				run_command(temp, 0);
				printf("Finished updating %s...\n", argv[4]);
			}
			else if(!strcmp(argv[4], "ramdisk.img" ) || !strcmp(argv[4], "ramdisk-uboot.img")) {
				sprintf(temp, "movi write rootfs 40A00000 %lx",size);
				run_command(temp, 0);
				printf("Finished updating %s...\n", argv[4]);
			}
			...
			*
			* Robin: TBD
			else if(!strcmp(argv[4], "system.img")) {
					//sprintf(temp, "");
					//run_command(temp, 0);
					//printf("Finished updating %s...\n", argv[4]);
					printf("%s need to be implement...\n",argv[4]);
			}
			else if(!strcmp(argv[4], "userdata.img")) {
					sprintf(temp, "");
					run_command(temp, 0);
					printf("Finished updating %s...\n", argv[4]);
			}
			*/
			else if(!strcmp(argv[4], "bl1ah")) {
				printf("%s...\n",argv[4]);
				
			}
			else		
				printf ( "[cmd_fat]:Not a valid image name for android .\r\n" );
